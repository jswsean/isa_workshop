...
$$
\begin{aligned}
Change_{TC} = \hat{TreatmentEffect} = & (Cholera_{L, POST} - Cholera_{L, PRE}) - \\
...
$$
\begin{aligned}
TreatmentEffect_{TC} = & (Outcome^{post}_{T} - Outcome^{pre}_{T}) - \\
...
But it can be shown that:
  ...
  $$Change_{TC} = TreatmentEffect_{TC} + NonParallelTrendBias_{TC}$$
---
# Imagine...
